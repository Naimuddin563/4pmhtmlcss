...
{
    mainDiv.textContent=''
    movies.forEach((m,ind) =>
    {
        const movieDiv = document.createElement("div") //<div></div>
        movieDiv.classList.add('movie') //<div class="movie"></div>
        const movieImage = document.createElement("img")
        movieImage.src = m.Images[0];
        movieImage.alt= m.Title
        const movieTitle = document.createElement("h2")
        movieTitle.textContent = m.Title;
        const movieDeleteButton = document.createElement('button')
        movieDeleteButton.textContent = "Delete"
        movieDeleteButton.onclick = function ()
        {
            movies.splice(ind, 1);
            displayData();
        }
        movieDiv.append(movieImage, movieTitle,movieDeleteButton)
        mainDiv.appendChild(movieDiv)
   })
}
displayData()

// const names=["raj","prasad","shekhar","kumar"]
// function displayData2 ()
// {
//     names.forEach((n) =>
//     {
//         const para = document.createElement('p');
//         para.textContent = "My name is " + n
